...
### 4. AI 말씀 추천 — 환각 없이, 정죄 없이, 개인정보 없이

작성한 회개·결단 기록에 어울리는 성경 구절을 추천하는 기능입니다. 단순히 "LLM에게
성경 구절을 물어보는" 방식은 세 가지 문제가 있었습니다. **없는 장절을 지어내고**,
**이미 회개하는 사람에게 다시 책망하는 구절을 들이밀고**, **내밀한 고백이 외부 API로
그대로 나갑니다.** 이 세 가지를 막는 데 집중했습니다.

#### 큐레이션한 구절 DB — 303개 구절, 5개 축으로 분류

AI가 성경 구절을 생성하지 못하게 하려면, **고를 수 있는 후보를 미리 준비**해야 합니다.
성경 39권에서 **303개 장절**을 직접 선별하고 다섯 가지 축으로 태깅했습니다.

| 축 | 개수 | 예시 |
|---|---|---|
| **주제**(themes) | 27종 | 회개 · 겸손 · 정결 · 용서 · 감사 · 소망 · 순종 · 절제 · 십자가 · 새사람 … |
| **상황**(situationTags) | 32종 | 죄고백 · 관계갈등 · 계획맡김 · 말씀회복 · 자기정죄 … |
| **감정**(emotionTags) | 15종 | 죄책감 · 수치심 · 두려움 · 낙심 · 무기력 · 분노 · 외로움 · 공허함 … |
| **관점**(perspectiveKey) | 27종 | 회개돌이킴 · 교만겸손 · 두려움담대함 · 정욕정결 · 관계용서 … |
| **톤**(tone) | 8종 | 권면(119) · 결단(46) · 위로(36) · 회복(32) · 지혜(21) · 소망(21) · 감사(15) · **책망(10)** |

여기에 **구체성**(1~5, 5일수록 특정 상황에 정확히 맞음)과 **회피 상황**(`avoidTags` —
이 구절을 추천하면 오히려 부담이 될 상황)을 함께 기록했습니다.

#### Gemini API 2회 호출 — 찾기(임베딩)와 고르기(생성)를 분리

추천 1건에 Gemini를 **두 번** 호출합니다. 역할을 나눈 이유는, 303개를 전부 프롬프트에
넣으면 토큰이 과하고 판단도 흐려지기 때문입니다. **좁히는 일은 벡터 검색이, 고르는 일은
생성 모델이** 하도록 분리했습니다.

**1차 — 임베딩 API** (`gemini-embedding-001`)
기록 텍스트를 768차원 벡터로 변환합니다. 구절 쪽 임베딩은 런타임이 아니라 **시드 시점에
미리 계산**해 저장해 두었습니다(장절·주제·상황·감정·요약·관점·톤을 한 문장으로 조합해
임베딩). 덕분에 런타임에는 사용자 텍스트 1건만 벡터화하면 됩니다. 동일 텍스트는 재임베딩을
건너뛰어 시드 비용도 아꼈습니다.

**pgvector 코사인 검색** — Gemini 호출 사이에 DB에서 의미가 가까운 **후보 24개**를 추립니다.

```sql
select ..., (1 - (v."embedding" <=> $1::vector(768))) as similarity
from verse v
where v."embedding" is not null
order by v."embedding" <=> $1::vector(768), v."weight" desc
limit 24
```

**2차 — 생성 모델** (`gemini-2.5-flash`)
후보 24개를 **압축한 JSON**(장절 텍스트·딥링크·가중치 등은 제외하고 판단에 필요한 필드만)과
사용자 기록을 함께 주고, **후보의 `verseId` 만 2~3개 고르게** 합니다. 구조화 출력
(`responseSchema`)으로 JSON 형식을 강제합니다.

#### 책망·정죄 구절을 피하는 3중 장치

이 기능에서 가장 신경 쓴 부분입니다. **이미 스스로 죄를 고백하며 회개한 사람에게
"너는 잘못했다"는 구절을 추천하면, 위로가 아니라 상처가 됩니다.**

**① 프롬프트에서 역할과 목적을 못 박기**

```
이 카드의 작성자는 이미 자기 부족을 인지하고 회개/결단의 마음으로 글을 쓴 사람이다.
추천의 목적은 같은 죄를 다시 정죄하거나 단죄하는 것이 아니라,
회복과 돌이킴의 자리에 함께 서도록 돕는 것이다.
```

**② 톤 기반 선택 규칙** — 구절마다 매긴 `tone` 을 근거로 명시적인 규칙을 줬습니다.

```
- tn="책망" 인 후보는 가능한 한 선택하지 않는다.
  작성자가 이미 자기 부족을 고백·회개하는 톤이라면 절대 선택하지 않는다.
  다만 카드 본문이 명백한 죄를 회피·정당화하는 경우에 한해 예외적으로 1개까지 허용한다.
- 가능하면 tn="회복"/"위로"/"권면"/"소망" 톤을 우선해 회복과 돌이킴의 방향을 제시한다.
- 훈계조/정죄조/예언적 단정 표현 금지.
- '하나님이 반드시 … 하신다' 같은 단정 표현 금지.
```

실제로 303개 중 **책망 톤은 10개(3%)** 뿐이고, 회복·위로·소망·권면 계열이 대부분을
차지하도록 큐레이션 단계에서부터 비중을 조절했습니다.

**③ 회피 상황 태그** — 예를 들어 "시편 51:1-4"에는 `avoidTags: ["심한자기정죄"]` 를
달아, 자책이 심한 상태에서는 오히려 부담이 될 수 있음을 데이터에 남겼습니다.

#### 서버 측 검증 — AI 응답을 그대로 믿지 않는다

생성 모델의 응답은 **반드시 서버에서 다시 검증**한 뒤에야 저장됩니다.

- **후보 밖 ID 폐기** — 응답의 `verseId` 가 후보 24개에 없으면 버립니다 (환각 차단)
- **장절·딥링크 재부착** — AI가 보낸 텍스트를 쓰지 않고 **DB 값으로 다시 붙입니다**
- **관점 중복 제거** — 같은 `perspectiveKey` 가 이미 선택됐으면 스킵해, 비슷한 말씀
  2~3개가 나오지 않게 합니다
- **범용 구절 제한** — 두루뭉술한 위로 구절(`isGeneric`)은 최대 1개까지만 허용
- **부족분 보충** — 검증 후 2개 미만이면 후보 상위에서 채우되, 위 다양성 규칙을 동일 적용

#### 그 외 안전장치

- **개인정보 마스킹** — 외부 API 전송 전 이메일·전화번호·URL·긴 숫자열을 플레이스홀더로
  치환합니다.
- **단계별 폴백** — 임베딩이 실패하면 가중치 기반 후보로, 생성 모델이 503/429 를 뱉으면
  다음 모델로 자동 재시도해 추천이 통째로 실패하지 않게 했습니다.
- **본인 전용 보장** — 추천은 별도 API 라우트에서 **작성자 본인인지 확인 후**에만
  내려줍니다. 피드 쿼리에는 아예 포함되지 않습니다.
- **테스트로 고정** — 후보 밖 ID 폐기, 다양성 규칙, 부족분 보충 등 검증 로직을 테스트
  코드로 묶어두었습니다. (`lib/scripture/recommendation.test.ts`)
...
### 6. 운영 중 새 기능 개발 — 운영/개발 환경 분리

AI 말씀 추천은 **이미 서비스가 운영되는 중에** 개발한 기능이었습니다. 실제 사용자들이
매일 기록을 남기고 있는 상황에서, 검증되지 않은 AI 기능 때문에 서비스가 멈추거나
실제 데이터가 오염되면 안 됐습니다. 그래서 개발과 운영을 완전히 분리했습니다.

- **브랜치 분리** — 운영(`master`) / 개발(`develop`) 브랜치를 나눠, 운영 중인 서비스는
  안정된 코드만 유지하고 새 기능은 개발 브랜치에서 작업했습니다.
- **배포 환경 분리** — Vercel 의 브랜치별 배포를 활용해 **개발 브랜치 전용 배포 URL**을
  따로 두고, 실제 기기에서 충분히 테스트한 뒤 운영에 반영했습니다.
- **DB 분리** — 개발용 DB를 별도로 만들어 환경별 연결 정보(`DATABASE_URL`)로 바라보게
  했습니다. 시드 스크립트도 dev/prod 환경 파라미터를 받도록 바꿔, 개발 중 스키마 변경이나
  잘못된 시드 실행이 **운영 데이터에 절대 닿지 않도록** 했습니다.

이 구조 덕분에 임베딩 스키마 추가(pgvector), 추천 파이프라인 교체 같은 큰 변경을
운영 서비스에 영향 없이 진행하고, 완성된 뒤 화이트리스트로 단계적으로 열 수 있었습니다.
...
- **04.28–05.03** — **AI 말씀 추천** MVP(Gemini) → 임베딩 기반 추천 고도화, 화이트리스트
  *(운영 중 개발 — 브랜치·배포·DB를 운영/개발로 분리한 뒤 진행)*
